...
class MedicalReportsController extends Controller
{
    ...
    public function index(Request $request)
    {
        if($request->ajax())
        {
            ...
            {
                $model=Group::query()
                ->with('patient','tests','cultures','contract','signed_by_user','created_by_user')->whereHas('contract' , function($q){
                    $q->where('id' , auth()->guard('admin')->user()->lab_id);
                });
            } elseif(auth()->guard('admin')->user()->address != null)
            {

                $model=Group::query()
                ->with('patient','tests','cultures','contract','signed_by_user','created_by_user')->where('doctor_id' , auth()->guard('admin')->user()->id);
            } 
            else
            {
                $model=Group::query()
...
public function print_report_2($id)
    {
        $group=Group::findOrFail($id);
    ...
    public function getComment(Request $request)
    {
        ...
        return response()->json([
            'comments' => TestComment::where([['case_id', $case] , ['component_id' , $request['component_id']]])->get()
        ]);
    ...
    public function addComment(Request $request)
    {
        ...
        foreach($request->status as $key => $status)
        {
            ...
            $comments[$key] = TestComment::where('test_id' , $request['test_id'])->whereHas('components', function($query) use($case , $request , $key){
                $query->where([['case_id', $case] , ['component_id' , $request['component'][$key]]]);
            })->first();
